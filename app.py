        ...
        if up_dau is not None:
            ...
            if len(mau_s) > 1:
                ...
                if dau_s is not None and len(ov) > 1:
                    ...
                    with tL:
                        st.markdown(f'<div style="{_tstyle}">월별 추이 — MAU × 스티키니스</div>', unsafe_allow_html=True)
                    with tR:
                        st.markdown(f'<div style="{_tstyle}">전년 동월 비교 —</div>', unsafe_allow_html=True)
                    with tW:
                        ymet = st.radio("비교 지표", ["스티키니스", "DAU", "MAU"], horizontal=True,
                                        key="yoy_metric", label_visibility="collapsed")
                    cL, cR = st.columns(2)
                    with cL:
                        figd = go.Figure()
